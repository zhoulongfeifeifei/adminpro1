(function(config) {
  ...
  function ha() {
    ...
    var a = a.toLowerCase(),
      b = null;
    if ((b = a.match(/angle \((.*)\)/)))
      (a = b[1]), (a = a.replace(/\s*direct3d.*$/, ""));
    a = a.replace(/\s*\([^\)]*wddm[^\)]*\)/, "");
    if (0 <= a.indexOf("intel")) {
      b = ["Intel"];
      0 <= a.indexOf("mobile") && b.push("Mobile");
      (0 <= a.indexOf("gma") || 0 <= a.indexOf("graphics media accelerator")) &&
        b.push("GMA");
          ...
          : a.match(/gts\b/)
            ...
            : a.match(/gt\b/)
              ...
              : a.match(/gs\b/)
                ...
                : a.match(/ge\b/)
                  ? b.push("GE")
                  : a.match(/fx\b/) && b.push("FX"),
        (c = a.match(ga)) && b.push(c[1].toUpperCase().replace("GS", "")),
        0 <= a.indexOf("titan")
          ? b.push("TITAN")
          : 0 <= a.indexOf("ti") && b.push("Ti"),
        (b = b.join(" ")))
      : 0 <= a.indexOf("amd") ||
        0 <= a.indexOf("ati") ||
        0 <= a.indexOf("radeon") ||
        ...
        ? ((b = ["AMD"]),
          ...
          0 <= a.indexOf("firepro")
            ? b.push("FirePro")
            : 0 <= a.indexOf("firegl") && b.push("FireGL"),
          0 <= a.indexOf("hd") && b.push("HD"),
          0 <= c && (a = a.substring(c)),
          (c = a.match(ga)) && b.push(c[1].toUpperCase().replace("HD", "")),
          (b = b.join(" ")))
        : a.substring(0, 100);
  }
  var ja = "microsoft basic render driver;vmware svga 3d;Intel 965GM;Intel B43;Intel G41;Intel G45;Intel G965;Intel GMA 3600;Intel Mobile 4;Intel Mobile 45;Intel Mobile 965".split(
    ...
    la =
      ("devicePixelRatio" in e && 1 < e.devicePixelRatio) ||
      (ka &&
        "matchMedia" in e &&
        e.matchMedia("(min-resolution:144dpi)") &&
        e.matchMedia("(min-resolution:144dpi)").matches),
    oa = T("windows nt"),
  ...
  var La = Ga ? "android" : Ba ? "ios" : oa ? "windows" : va ? "mac" : "other",
    ...
    kb =
      e.navigator && e.navigator.pointerEnabled && !!e.navigator.maxTouchPoints,
    lb = kb || jb,
    ...
    nb = (function() {
      ...
      for (var c = a.length - 1; 0 <= c; c -= 1)
        if ("viewport" === a[c].name) {
          var c = a[c].content,
            d;
          -1 !== c.indexOf("initial-scale") &&
            (d = parseFloat(c.split("initial-scale=")[1]));
          a =
            -1 !== c.indexOf("minimum-scale")
              ? parseFloat(c.split("minimum-scale=")[1])
              : 0;
          c =
            -1 !== c.indexOf("maximum-scale")
              ? parseFloat(c.split("maximum-scale=")[1])
              : Infinity;
          if (d) {
    ...
    qb =
      !!da.createElementNS &&
      !!da.createElementNS("http://www.w3.org/2000/svg", "svg").createSVGRect,
    ...
    Ab = (function() {
      ...
      for (
        var a = ["webgl", "experimental-webgl", "moz-webgl"], b = null, c = 0;
        c < a.length;
        c += 1
      ) {
        try {
        ...
        if (b) {
          if (
            b.drawingBufferWidth !== rb.width ||
            b.drawingBufferHeight !== rb.height
          )
            break;
          ...
          if (f > yb) break;
          if (
            23 > b.getShaderPrecisionFormat(35632, 36338).precision ||
            23 > b.getShaderPrecisionFormat(35633, 36338).precision
          )
            break;
          ia = b.getExtension("WEBGL_debug_renderer_info")
            ? b.getParameter(37446)
            : null;
  ...
  config.j.ne &&
    ((config.Fd += "v"),
    (Ub += ",vectorlayer,overlay"),
    config.j.Zd
      ? ((config.Fd += "w"), (Ub += ",wgl"))
      : ((config.Fd += "cg"), (Ub += ",cmng,cgl")));
  if (config[7]) {
    for (
      var Vb = [], Wb = config[7].split(","), Xb = 0;
      Xb < Wb.length;
      Xb += 1
    ) {
      var Yb = Wb[Xb];
  ...
  try {
    if (window.localStorage) {
      ...
      tc
        ? ((tc = JSON.parse(tc)),
          tc.version === config.wg
            ? (eval(tc.script), (Zb.loaded = !0))
            : (uc = !0))
        : (uc = !0);
      if (uc)
        for (Xb in window.localStorage)
          window.localStorage.hasOwnProperty(Xb) &&
            0 === Xb.indexOf("_AMap_") &&
            window.localStorage.removeItem(Xb);
    }
  } catch (vc) {}
  Zb.loaded || (rc(), delete config.AN);
})([
  "45207a2f736cc4fba2507bd0b1e91bec",
  [118.344933, 29.188757, 120.721945, 30.566516, 120.153576, 30.287459],
