  ...
  <div class="box2" v-show="pData.show">
    <div class="box2con change" v-show="boxShow">
      ...
      <div class="form-group mb30">
        ...
        <a type="button" class="pointer mr35" @click="Submit(true)" :disabled="pData.disabled">确定</a>
    ...
    AuthTree(object, name, index){
      ...
      Api.AuthTree({
        ...
        if(sj.status==0){
          ...
          Swal({title:"地点权限列表服务器返回错误", text:sj.errorMsg, type:"error"})
        }
      }).catch(function(error) {
        Swal("地点权限列表请求服务器失败", "", "error");
      })
    },
    Arr(object, name){
      let self=this, i=0;
