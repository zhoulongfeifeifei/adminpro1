...
export default{
  ...
  props:["show", "memberLevel"],
  watch:{
    "show":function(nVal, oVal){
      if(nVal==true){
        this.boxShow=true;
        this.pData=this.memberLevel;
    ...
    AuthTree(object, name, index){
      ...
      Api.AuthTree({
        ...
        if(sj.status==0){
          ...
          Swal({title:"等级权限列表服务器返回错误", text:sj.errorMsg, type:"error"})
        }
      }).catch(function(error) {
        Swal("等级权限列表请求服务器失败", "", "error");
      })
    },
    Arr(object, name){
      let self=this, i=0;
