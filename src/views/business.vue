<template>
  <section class="con">
    ...
    <businessAuth :show="pData.show" :business="pData.business" @Close="Close"></businessAuth>
  </section>
</template>
    ...
    Initial(){                                                                                   // 初始化
      ...
      Api.Business(
        ...
        if(sj.status==0){
          ...
          Swal({title:"业态列表服务器返回错误", text:sj.errorMsg, type:"error"})
        }
      }).catch(function(error) {
        Swal("业态列表请求服务器失败", "", "error")
      })
    },
    EditAuth(business){
      this.pData.show=true;
      this.pData.business=business;
    },
    Close(msg){
